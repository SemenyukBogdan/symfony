...
class RequestCheckerService{
    ...
    public function check(mixed $content, array $fields): bool
    {
        ...
        if ($errors) {
            throw new BadRequestException('Required fields are missed: ' . $errors,
                Response::HTTP_BAD_REQUEST);
        }
        return true;
    }
     ...
     * @return void
     */
    public function validateRequestDataByConstraints(array|object $data, ?array
                                                                  $constraints = null, ?bool $removeSquareBracketFromPropertyPath = false): void
    {
        $errors = $this->validator->validate($data, !empty($constraints) ? new
